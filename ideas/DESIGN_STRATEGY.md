# aphype Architectural Design & Development Strategy

> **aphype: The Social Influence Engine for AI-Native Content.**

This document records the core architectural decisions, technical selection criteria, and integration paths within the `aipartnerup` ecosystem for `aphype` (formerly `apforge`).
...
- **Naming Rationale**:
  - Renamed to **`aphype`** to perfectly fit the "4-letter component" sequence of the `aipartnerup` ecosystem (`apcore`, `apflow`).
...
`aphype` supports two modes: "Local Direct Publishing" and "Cloud Bridging", which are completely transparent to the user.

### 3.1 Native Providers
- **Scenario**: For developer-friendly platforms like Dev.to, Hashnode, and GitHub.
- **Implementation**: `aphype` -> Platform Official API (user-provided API Key).
...
- **Implementation**: `aphype` -> **Postiz Cloud / Third-party SaaS API** -> Target platform.
...
- **Layered Storage**:
  - `~/.config/aphype/credentials.yaml`: Global private tokens (Permission: 600).
...
`aphype` strictly follows the principle of "Everything is a File" and "User Visible, Controllable, and Repairable".
...
- [ ] **Phase 5**: Implement `aphype watch` automated daemon process.

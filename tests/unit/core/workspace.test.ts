...
beforeEach(async () => {
  tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), 'aphype-ws-'));
  delete process.env.APHYPE_WORKSPACE;
});
...
describe('WorkspaceResolver.resolve', () => {
  ...
  test('Step 1: APHYPE_WORKSPACE env var resolves workspace', async () => {
    await createWorkspace(tmpDir, ['blog']);
    process.env.APHYPE_WORKSPACE = tmpDir;
  ...
  test('Step 2: walks up from cwd to find .aphype/', async () => {
    await createWorkspace(tmpDir, ['blog']);
  ...
  test('Step 3: cwd contains project.yaml, parent has .aphype/', async () => {
...
describe('WorkspaceResolver.isWorkspace / isProject', () => {
  test('isWorkspace returns true when .aphype/config.yaml exists', async () => {

...
| **Priority** | P0                                                 |
| **SRS Refs** | New (supports `aphype refine` multi-turn)          |
...
```
{projectDir}/.aphype/sessions/{article}/{stage}.json
```
...
export class SessionManager {
   ...
   *   Must be non-empty, absolute. Session files stored under {projectDir}/.aphype/sessions/.
   */
   ...
   *
   * Creates parent directories (.aphype/sessions/{article}/) if they don't exist.
   ...
   * @param article - Article name.
   *
   * Removes the entire {article}/ directory under .aphype/sessions/.
...
3. Compute `sessionsDir = path.join(projectDir, ".aphype", "sessions")`.
...
```json
{
  ...
  "adapter": "claude",
  "stage": "draft",
  "cwd": "/Users/alex/aphype-workspace/my-project",
...
*SRS Traceability: NFR-REL-002 (idempotent operations - atomic writes prevent corruption), supports US-003 enhanced (iterative refinement workflow). Session management is new functionality not directly mapped to existing SRS requirements; it enables the `aphype refine` command.*

...
The watcher daemon is a long-running background process that periodically checks `05_scheduled` for due items and auto-publishes them. It reuses the same publish logic as `aphype publish` but runs continuously with configurable intervals, signal handling, and file logging.
...
3. Log: "[timestamp] INFO: aphype watcher started. Check interval: {intervalMinutes} minutes."
...
Log file: `{workingDir}/aphype-watcher.log`

Each entry is one line with format:
```
[2026-03-14T10:00:00.000Z] INFO: aphype watcher started. Check interval: 60 minutes.

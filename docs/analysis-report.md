...
aphype 项目拥有 6 份核心文档，涵盖了产品定位、技术设计、操作流程和开发策略。文档集合的主要问题是：`ideas/DESIGN_STRATEGY.md` 与 `ideas/aphype/draft.md` 是完全相同的副本，`pyproject.toml` 中残留了已弃用的 Python 技术栈描述与当前 Bun/TypeScript 架构严重矛盾，且缺少测试计划、API 规范和部署指南等关键文档。
...
| `docs/project-aphype.md` | 产品功能清单（Feature Manifest） | 当前有效 |
| `docs/tech-design-core.md` | Phase 1 核心编排技术设计 | 当前有效，但内容偏少 |
| `ideas/DESIGN_STRATEGY.md` | 架构设计与开发策略 | 与 draft.md 完全重复 |
| `ideas/aphype/draft.md` | 架构设计与开发策略 | 与 DESIGN_STRATEGY.md 完全重复 |
...
### 主题 1：产品定位与命名
- `README.md` — 高层定位描述
- `ideas/DESIGN_STRATEGY.md` — 命名由来（apforge → aphype）
- `docs/project-aphype.md` — 功能定位
...
> `README.md`: "aphype recommends using Bun for extreme performance and a single-file distribution experience."
> `package.json`: 实际依赖为 `commander`, `chalk`, `@google/generative-ai` 等 Node/Bun 包。

此外，`pyproject.toml` 的 description 使用了旧名 **"AI PartnerUp Press"**，而 `package.json` 已更新为 **"AI PartnerUp Hype"**。
...
#### 冗余 1：`ideas/DESIGN_STRATEGY.md` 与 `ideas/aphype/draft.md` 完全相同
...
| "AI PartnerUp Press" 名称 | `pyproject.toml` | 项目已更名为 "AI PartnerUp Hype" |
...
2. **【紧急】删除 `ideas/aphype/draft.md`** — 与 `ideas/DESIGN_STRATEGY.md` 完全重复，保留一份即可。
...
6. **【中优先级】统一 Roadmap** — 合并 `docs/project-aphype.md` 和 `ideas/DESIGN_STRATEGY.md` 中的两个不同 Roadmap 为单一权威版本。

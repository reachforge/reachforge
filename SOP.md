...
As the Editor-in-Chief of `reachforge`, your task is to manage "file flow". AI is responsible for performing "content transformation".

### 🛠️ CLI Commands

1. **`reachforge status`** (View Dashboard)
   - Scan `01-06` directories.
   - Count current "Work in Progress" (WIP) items.
   - Remind tasks to be published today.

2. **`reachforge draft <source>`** (Generate Draft)
   - Read from `01_inbox`.
   - Call AI to generate a long-form article and save it to `02_drafts`.

3. **`reachforge adapt <article>`** (Multi-platform Adaptation)
...
4. **`reachforge schedule <article> <date>`** (Set Schedule)
   - Move the adapted directory to `05_scheduled`.
   - Force rename to `YYYY-MM-DD-title` format.

5. **`reachforge publish`** (Execute Distribution)
